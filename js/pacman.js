...
//Definir el tamaño de la grilla y cómo obtener la posición
...
//Definir prototipos de objetos en el juego
class GameObject{
  constructor(args){    
    let def = {
      p: new Vec2(0,0),
...
class Player extends GameObject{
  ...
  moveStep(){
    ...
    //Mira dónde puedes moverte
    let avail = ['up','down','left','right']
                .filter(d=>!haveWall[d])
    //Actualizar la dirección si no hay un muro en la próxima dirección especificada
    if (!haveWall[this.nextDirection]){
      this.currentDirection=this.nextDirection   
    }
    //Actualizar la ubicación según la dirección
    ...
    if (!isWall){
      ...
      //Si está en los bordes izquierdo y derecho y en la dirección => salto instantáneo
      ...
      //Si la siguiente dirección es la pared, retrocederá a la posición original y mantendrá la dirección original.
...
class Pacman extends Player{
  ...
  die(){
    if (!this.isDead){
      //Restablecer todas las animaciones
      TweenMax.killAll()
      this.isDead=true
...
class Ghost extends Player{
  ...
  getNextDirection(map,pacman){
    ...
    //Deje la dirección que debería seguir
    let traceGo = this.traceGoCondition.filter(obj=> {
      let cond = obj.condition(currentTarget) 
      return go? cond:!cond
    }).map(obj=>obj.name) 
    
    ///Obtén la dirección a la que puedes ir
    let haveWall = map.getWalls(this.gridP.x,this.gridP.y)
    
    //El filtrado puede avanzar y debe ir en la dirección opuesta
    ...
    //Filtra la dirección en la que puedes ir
    let availGo =['left','right','up','down']
              .filter(d=>!haveWall[d])
    
    //Si solo hay dos direcciones en el momento, mantenga el original
    if (availGo.length==2){
    ...
    //Da prioridad a la dirección del movimiento, si no, elige de la dirección
...
function update(){
  ...
  map.ghosts.forEach(ghost=>{
    ...
    //Estrategia de caminar fantasma
    ...
    if (!ghost.isDead && !map.pacman.isDead && ghost.collide(map.pacman)){
      if (!ghost.isEatable){
        //Fantasma comiendo elfo
        map.pacman.die()
        setTimeout(()=>{
          map.init()
        },4000)
      }else {
        //Elfo comiendo fantasma
        ghost.die()
        //Suspender
        TweenMax.pauseAll()
        setTimeout(()=>{
...
window.addEventListener("keydown",function(evt){
  //Establecer la dirección del elfo
